...
MainWindow::~MainWindow()
{
    ...
    for (int var = 0; var < savedSessions.length(); ++var) {
        delete savedSessions.operator [](var);
    }
    savedSessions.clear();
    delete (*states)[0];
    delete (*states)[1];
...
void MainWindow::setUpStatesMenus(){
    ...
    states->append(new SessionSummaryState("", "Summary"));
      ...
      case 5:
        ...
        if (index ==1){
            ...
            if (currentState->getCurrentIndex()!=-1){
                delete savedSessions.operator [](currentState->getCurrentIndex());
